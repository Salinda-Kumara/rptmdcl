...
export async function buildApplicationPacket(app: StaffApplication): Promise<Blob> {
  ...
  const generatedText = `Generated: ${new Date().toLocaleString('en-GB')}`;
 ...
 */
export async function printApplicationPacket(app: StaffApplication, win?: Window | null) {
 ...
 */
export async function printApplicationById(id: string, win?: Window | null) {
  const w = win !== undefined ? win : openBlankTab();
  try {
    const full = await staffApi.getApplication(id);
    await printApplicationPacket(full, w);
  } catch (e) {
    try { w?.close(); } catch { /* noop */ }
    throw e;
  }
