...
export async function buildApplicationPacket(app: StaffApplication, generatedBy?: string): Promise<Blob> {
  ...
  // Pages we generated ourselves (form + image placeholders) get the footer
  // disclaimer stamped on them below. Raw attachment PDF pages are left
  // completely untouched — no footer, no label — so they stay byte-for-byte
  // identical to what the student uploaded.
  const stampablePages: any[] = [];

  // Draw a form page onto a fresh A4 sheet at native size (the generated form
  // pages are already A4 portrait).
  const placeFormPage = (ep: any) => {
    const page = merged.addPage([A4_W, A4_H]);
    page.drawPage(ep, { x: 0, y: 0, width: A4_W, height: A4_H });
    stampablePages.push(page);
  };

  // Draw an attachment PDF page unaltered: the new page matches the attachment's
  // own size and the content is drawn at 1:1 scale, so what staff see is exactly
  // what the student uploaded — no resizing, cropping, or label drawn over it.
  ...
  for (const d of app.documents ?? []) {
    ...
    if (isPdf) {
      ...
      // Images have no inherent "page" to preserve, so they're placed fitted
      // onto a generated A4 page (with a label identifying the attachment) —
      // this doesn't alter the image itself, unlike the PDF scaling this fix removes.
  ...
  // 3) footer on our own generated pages only — attachment PDF pages are left as-is
  ...
  drawPacketFooters(stampablePages, { font, fontItalic, rgb, serialText, generatedText });
