...
import { CalendarDays, Plus, Pencil, Trash2, Loader2, AlertCircle, ChevronRight } from 'lucide-react';
...
interface Props { onOpen?: (scheduleId: string) => void; }

export function ExamSchedulesPanel({ onOpen }: Props) {
  ...
  const save = async () => {
    setError('');
    if (!name.trim() || !startDate || !endDate) { setError('Name and dates are required'); return; }
    if (new Date(endDate) < new Date(startDate)) { setError('End date cannot be before start date'); return; }
    setSaving(true);
    try {
      const payload = { name, startDate, endDate, programmeId: programmeId || undefined, description };
    ...
    <div>
      ...
      : (
        <div className="space-y-3">
          {items.map((s) => (
            <div key={s.id} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-colors hover:border-amber-200">
              <button onClick={() => onOpen?.(s.id)} className="flex min-w-0 flex-1 items-center gap-4 text-left" title="Open timetable">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-rose-50 text-rose-600">
                  <CalendarDays className="h-5 w-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-slate-900">{s.name}</p>
                  <p className="mt-0.5 text-xs text-slate-500">{fmt(s.startDate)} → {fmt(s.endDate)}{s.programmeId ? ` · ${progName(s.programmeId)}` : ''}</p>
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => onOpen?.(s.id)} className="rounded-lg p-2 text-slate-400 hover:bg-amber-50 hover:text-amber-600" title="Open timetable"><ChevronRight className="h-4 w-4" /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600">Programme <span className="font-normal text-slate-400">(optional — leave blank for a combined timetable)</span></label>
            <select value={programmeId} onChange={(e) => setProgrammeId(e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
              <option value="">— None (all programmes) —</option>
