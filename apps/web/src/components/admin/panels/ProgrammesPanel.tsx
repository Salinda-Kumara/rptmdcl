...
export function ProgrammesPanel() {
  ...
  const load = () => { setLoading(true); adminApi.listProgrammes().then(setItems).catch(() => {}).finally(() => setLoading(false)); };
  useEffect(load, []);

  const openCreate = () => { setEditing(null); setCode(''); setName(''); setDescription(''); setError(''); setModalOpen(true); };
  const openEdit = (p: AdminProgramme) => { setEditing(p); setCode(p.code); setName(p.name); setDescription(p.description || ''); setError(''); setModalOpen(true); };
  ...
  const remove = async (p: AdminProgramme) => {
    ...
    await adminApi.deleteProgramme(p.id); load();
  };
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900"><GraduationCap className="h-6 w-6 text-amber-500" /> Programmes</h1>
      ...
      {loading ? <div className="grid gap-4 sm:grid-cols-2">{[0,1].map((i) => <div key={i} className="h-32 animate-pulse rounded-2xl bg-white" />)}</div>
      : items.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-sm text-slate-400">No programmes yet.</div>
      : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Programme' : 'New Programme'}
        footer={<>
