...
import { Users, GraduationCap, BookOpen, Layers, CalendarDays, FileText, Wallet, ArrowRight, TrendingUp } from 'lucide-react';
...
export function AdminDashboardPanel({ onNavigate }: Props) {
  ...
  const cards = [
    { label: 'Staff Users',    value: stats?.totalStaff,        icon: Users,        view: 'users',      tint: 'bg-blue-50 text-blue-600' },
    { label: 'Students',       value: stats?.totalStudents,      icon: GraduationCap, view: 'students',   tint: 'bg-violet-50 text-violet-600' },
    { label: 'Programmes',     value: stats?.totalProgrammes,    icon: GraduationCap, view: 'programmes', tint: 'bg-emerald-50 text-emerald-600' },
    { label: 'Subjects',       value: stats?.totalSubjects,      icon: BookOpen,      view: 'subjects',   tint: 'bg-amber-50 text-amber-600' },
    { label: 'Batches',        value: stats?.totalBatches,       icon: Layers,        view: 'batches',    tint: 'bg-cyan-50 text-cyan-600' },
    { label: 'Exam Schedules', value: stats?.totalSchedules,     icon: CalendarDays,  view: 'schedules',  tint: 'bg-rose-50 text-rose-600' },
  ];
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {cards.map((c) => {
          const Icon = c.icon;
          return (
            <button key={c.label} onClick={() => onNavigate(c.view)}
              className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-amber-200 hover:shadow-md text-left">
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 p-6 shadow-sm">
            ...
            <p className="text-2xl font-bold text-emerald-700">{loading ? '…' : formatFee(stats?.verifiedRevenue ?? 0)}</p>
