...
export function DashboardPanel({ onNavigate }: Props) {
  ...
  // Verification officers (full application review, but not payments/admin) get an
  // exam-division dashboard focused on the review queue.
  const isVerifierOnly = !isAdmin && permissions.applications === 'FULL' && !can(permissions, 'payments', 'FULL');

  const [stats, setStats] = useState<StaffStats | null>(null);
  const [queue, setQueue] = useState<StaffApplication[]>([]);
  const [financeApps, setFinanceApps] = useState<StaffApplication[]>([]);
  const [verifierApps, setVerifierApps] = useState<StaffApplication[]>([]);
  ...
  useEffect(() => {
    ...
    } else if (isVerifierOnly) {
      staffApi.getApplications({}).then(setVerifierApps).catch(() => {}).finally(() => setLoading(false));
    } else {
      Promise.all([
  ...
  }, [permsLoading, isFinanceOnly, isVerifierOnly]);
  ...
  /* ──────────────── Verification (Exam Division) dashboard ──────────────── */
  if (isVerifierOnly) {
    const toVerify = verifierApps.filter((a) => a.status === 'SUBMITTED');
    const forwarded = verifierApps.filter((a) => ['PAYMENT_PENDING', 'PAYMENT_VERIFIED', 'APPROVED'].includes(a.status));
    const rejected = verifierApps.filter((a) => a.status === 'REJECTED');

    const verCards = [
      { label: 'To Verify', value: toVerify.length, icon: Clock, ring: 'bg-amber-50 text-amber-600', tint: 'from-amber-500 to-orange-500' },
      { label: 'Forwarded to Finance', value: forwarded.length, icon: Wallet, ring: 'bg-indigo-50 text-indigo-600', tint: 'from-indigo-500 to-violet-500' },
      { label: 'Rejected', value: rejected.length, icon: XCircle, ring: 'bg-rose-50 text-rose-600', tint: 'from-rose-500 to-red-500' },
    ];
    ...
    return (
      <div>
        <div className="mb-7">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-gray-100">Verification Dashboard</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-gray-400">Application review overview.</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-gray-800 px-5 py-4">
            <div>
              <h2 className="text-base font-semibold text-slate-900 dark:text-gray-100">Awaiting Verification</h2>
              <p className="text-xs text-slate-400 dark:text-gray-500">Submitted applications to review and forward</p>
            </div>
            <button onClick={() => onNavigate('applications')} className="inline-flex items-center gap-1 text-sm font-medium text-indigo-600 hover:text-indigo-700">
              View all <ArrowRight className="h-3.5 w-3.5" />
            </button>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center px-5 py-12 text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-slate-100"><Inbox className="h-7 w-7 text-slate-400" /></div>
              <p className="mt-3 text-sm font-medium text-slate-700 dark:text-gray-300">Queue is clear</p>
              <p className="mt-1 text-sm text-slate-400 dark:text-gray-500">No applications awaiting verification.</p>
            </div>
          ) : (
            <ul className="divide-y divide-slate-100 dark:divide-gray-800">
              {toVerify.slice(0, 8).map((app) => (
                <li key={app.id}>
                  <button onClick={() => onNavigate('app-detail', app.id)} className="group flex w-full items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-slate-50 dark:hover:bg-gray-800/50">
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${app.type === 'MEDICAL' ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600'}`}><FileText className="h-5 w-5" /></div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-slate-900 dark:text-gray-100">{app.student?.fullName || 'Unknown student'}</p>
                      <p className="truncate text-xs text-slate-500 dark:text-gray-400">
                        {app.serialNumber ? `${app.serialNumber} · ` : ''}{app.student?.registrationNumber} · {app.type === 'MEDICAL' ? 'Medical' : 'Repeat'} · {app.applicationSubjects.length} subject(s)
                      </p>
                    </div>
                    <ChevronRight className="h-5 w-5 shrink-0 text-slate-300" />
                  </button>
