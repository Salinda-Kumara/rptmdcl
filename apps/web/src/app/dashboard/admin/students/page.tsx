...
import {
  UserSquare2, Upload, Search, Loader2, AlertCircle, CheckCircle2,
  FileSpreadsheet, X, ArrowRight,
} from 'lucide-react';
...
export default function AdminStudentsPage() {
  ...
  useEffect(() => {
    const t = setTimeout(() => loadList(true), search ? 350 : 0);
    return () => clearTimeout(t);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [search]);

  useEffect(() => { loadList(); /* eslint-disable-next-line */ }, [skip]);
  ...
  const runPreview = async (f: File) => {
    ...
    try {
      setPreview(await adminApi.importStudents(f, true));
    } catch (e: any) {
      setError(e.response?.data?.message?.toString() || 'Could not read the file');
  ...
  const runImport = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.message?.toString() || 'Import failed');
    } finally { setBusy(null); }
  };
    ...
    <AdminShell>
      <div className="mb-6">
        <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900">
          <UserSquare2 className="h-6 w-6 text-amber-500" /> Students
        </h1>
        ...
        {result && (
          <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-xs text-emerald-700">
              ...
              <span>Repeated reg-nos: <b>{result.duplicateRegNumbers}</b></span>
            ...
            {preview && (
              <div className="mt-4">
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {[
                    { label: 'Rows in file', value: preview.total },
                    { label: 'To import', value: preview.toImport },
                    { label: 'New', value: preview.willCreate },
                    { label: 'Update existing', value: preview.willUpdate },
                  ].map((s) => (
                ...
                <p className="mt-2 text-xs text-slate-400">
                  {preview.mergedDuplicatePersons} same-person duplicate(s) and {preview.duplicateRegNumbers} repeated registration number(s) will be merged.
                </p>
                ...
                <button
                  onClick={runImport}
                  disabled={busy !== null}
                  className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-amber-600 disabled:opacity-50"
                >
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            <span className="text-slate-500">
              {skip + 1}–{Math.min(skip + PAGE, total)} of {total.toLocaleString()}
            </span>
            <div className="flex gap-2">
              <button
                onClick={() => setSkip(Math.max(0, skip - PAGE))}
                disabled={skip === 0}
                className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-40"
              >
                Previous
              </button>
              <button
