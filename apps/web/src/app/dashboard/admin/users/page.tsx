...
export default function AdminUsersPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    setEmail(''); setName(''); setPosition(''); setPassword(''); setIsAdmin(false);
  ...
  const openEdit = (u: AdminUser) => {
    setEditing(u);
    setEmail(u.email); setName(u.staffUser?.name || ''); setPosition(u.staffUser?.position || '');
  ...
  const setLevel = (resource: string, level: Level) =>
    setMatrix((prev) => ({ ...prev, [resource]: level }));

  const grantsFromMatrix = () =>
    RESOURCES
      .filter((r) => matrix[r.key] && matrix[r.key] !== 'NONE')
      .map((r) => ({ resource: r.key, level: matrix[r.key] as AccessLevel }));
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await adminApi.updateUser(editing.id, { name, position, isAdmin, permissions, ...(password ? { password } : {}) });
      } else {
        await adminApi.createUser({ email, password, name, position, isAdmin, permissions });
      }
      setModalOpen(false);
      load();
    } catch (e: any) {
  ...
  return (
    <AdminShell>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900">
            <Users className="h-6 w-6 text-amber-500" /> Staff &amp; Permissions
          </h1>
          <p className="mt-1 text-sm text-slate-500">Create staff accounts and grant per-feature access (View / Full).</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-sm text-slate-400">No staff users yet.</div>
      ) : (
                ...
                <tr key={u.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                  ...
                  <td className="px-4 py-3.5">
                    {u.isAdmin ? (
                      ...
                      <div className="flex flex-wrap gap-1">
                        {u.permissions.map((p) => (
                          <span key={p.resource} className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium ${
                            p.level === 'FULL' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'
                          }`}>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => openEdit(u)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-amber-600" title="Edit"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => deactivate(u)} className="rounded-lg p-2 text-slate-500 hover:bg-red-50 hover:text-red-600" title="Deactivate"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="space-y-4">
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3">
            ...
            <span className="flex items-center gap-1.5 text-sm font-medium text-amber-800">
              <Crown className="h-4 w-4" /> Master Admin — full access to everything
            </span>
          </label>
