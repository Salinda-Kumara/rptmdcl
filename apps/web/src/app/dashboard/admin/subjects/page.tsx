...
export default function AdminSubjectsPage() {
  ...
  const load = () => {
    ...
    Promise.all([adminApi.listSubjects(), adminApi.listProgrammes()])
      .then(([s, p]) => { setSubjects(s); setProgrammes(p); })
      .catch(() => {})
  ...
  const openCreate = () => {
    setEditing(null);
    setCode(''); setName(''); setCategory('Core');
    setProgrammeId(programmeFilter || programmes[0]?.id || '');
    setError(''); setModalOpen(true);
  };
  const openEdit = (s: AdminSubject) => {
    setEditing(s); setCode(s.code); setName(s.name); setCategory(s.category); setProgrammeId(s.programmeId);
    setError(''); setModalOpen(true);
  };
    ...
    <AdminShell>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900">
            <BookOpen className="h-6 w-6 text-amber-500" /> Subjects
          </h1>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row">
        <select
          ...
          className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus:border-amber-400 focus:outline-none"
        >
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-sm text-slate-400">
          {subjects.length === 0 ? 'No subjects yet.' : 'No subjects match the filters.'}
        </div>
      ) : (
