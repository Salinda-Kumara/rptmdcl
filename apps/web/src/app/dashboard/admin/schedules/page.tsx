...
export default function AdminSchedulesPage() {
  ...
  const load = () => {
    ...
    Promise.all([adminApi.listSchedules(), adminApi.listProgrammes()])
      .then(([s, p]) => { setItems(s); setProgrammes(p); })
      .catch(() => {})
  ...
  const openCreate = () => {
    ...
    setProgrammeId(programmes[0]?.id || ''); setDescription(''); setError(''); setModalOpen(true);
  };
  const openEdit = (s: AdminSchedule) => {
    setEditing(s); setName(s.name); setStartDate(toInput(s.startDate)); setEndDate(toInput(s.endDate));
    setProgrammeId(s.programmeId); setDescription(s.description || ''); setError(''); setModalOpen(true);
  };
  ...
  return (
    <AdminShell>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900">
            <CalendarDays className="h-6 w-6 text-amber-500" /> Exam Schedules
          </h1>
          <p className="mt-1 text-sm text-slate-500">Define examination periods — students&apos; upcoming exam dates auto-fill from these.</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-2">{[0, 1, 2].map((i) => <div key={i} className="h-16 animate-pulse rounded-lg bg-white" />)}</div>
      ) : items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-sm text-slate-400">No exam schedules yet.</div>
      ) : (
            ...
            <div key={s.id} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  {fmt(s.startDate)} → {fmt(s.endDate)} · {progName(s.programmeId)}
                </p>
